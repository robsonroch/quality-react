...
function DetailForm(props) {
  ...
  const [resultColor, setResultColor] = useState("primary");

  const colorMapping = {
    ["não classificada"]: "primary",
    ["sucesso"]: "success",
    ["rejeitada"]: "error",
    ["recusada"]: "error",
  };
    ...
    if (token) {
      fetch(`http://localhost:8888/notificacoes/${id}`, {
        ...
        .then((dados) => {
          ...
          if (dados.resultClassification) {
            const color = Object.keys(colorMapping).find((key) =>
              dados.resultClassification.includes(key)
            );
            if (color) setResultColor(colorMapping[color]);
          }
  ...
  return (
    <div className="wrapper">
      ...
      <div className="d-flex align-items-baseline mb-1 ms-0">
        ...
        <span className={`m-1 badge bg-${resultColor}`}>
